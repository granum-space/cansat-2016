...
void sens_init() {
	...
	dht22 = rscs_dht22_init(&PORTB, &PINB, &DDRB, 4, 7.0f); //DHT22
...
void sens_update_so_slow() {
	...
	if(rscs_ds18b20_check_ready()) {//DS18B20
		telemetry_so_slow.ds18b20_error = rscs_ds18b20_read_temperature(ds18b20, &(telemetry_so_slow.temperature_ds18));
		rscs_ds18b20_start_conversion(ds18b20);
	}
