...
uint16_t _recalc(uint8_t angle) {
	if(angle >= 180) return GR_SERVO_MAX_VALUE;

	return GR_SERVO_MIN_VALUE + ((GR_SERVO_MAX_VALUE - GR_SERVO_MIN_VALUE) / 180.0 * angle);
}
...
void gr_servo_init() {
	...
	OCR1B = GR_SERVO_START_VALUE; // начальное положение сервы
